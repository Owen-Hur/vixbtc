...
|---|---|---|---|---|
| `rv_regime/` | BTC 자체 RV 레짐으로 방향을 잡을 수 있는가? | 편향 제거 시 Sharpe 0.69 → 0.02, 알파 소멸 | 기각 | ✅ 편향 제거판 완전 재현 (-57.4%, Sharpe 0.02, -109.0%p) / 편향판은 근사 재구성 |
| `vix_response/` | VIX 발표 후 BTC는 언제·어떻게 반응하는가? | 2020~2022엔 6h 피크, 2023+ 소멸 | 중간 단계 | ⏭ 미검증 (중간 단계 탐색 코드) |
...
| `other_signals/` | 펀딩비 · ETF flow · DVOL · F&G는? | 단독 예측력 없음 (펀딩 r = −0.005) | 부가 | ⏭ 미검증 (부가 신호) |
