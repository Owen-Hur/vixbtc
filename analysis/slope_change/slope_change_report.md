...
**관찰 3**: 평균 적중률은 ALL 46.29%, IS 46.29%, OOS 46.30% — 모두 **50% 이하**다.

> 2026-09-08 재실행 검증: 위 세 값은 `data/slope_change_summary.json` 의 `tday_window_avg` 와 소수점까지 일치한다.
> (초판 원고에 'IS ~47% / OOS ~49%'로 적혀 있던 것은 반올림 오기이며, 이번에 정정했다.)
...
| T-day 윈도우 분 단위 평균 적중률 | 46.29% | 46.30% | 46.29% |
