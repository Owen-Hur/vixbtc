...
w("")
w("> 작성일 2026-05-31 · 코드 `vix_granger.py`")
w("")
w("## 0. Granger 인과란")
...
w("")
rc=stats.pearsonr(dd["dvix_pct"],dd["btc_ret"]); rcs=stats.pearsonr(dd["dslope"].dropna(),dd.loc[dd["dslope"].notna(),"btc_ret"])
w(f"**참고 동시 상관(일별):** ΔVIX% vs BTC r={rc[0]:+.3f}(p={rc[1]:.1e}) · Δslope vs BTC r={rcs[0]:+.3f}(p={rcs[1]:.1e}) — 동시성은 유의(거래 불가).")
w("")

R={}
w("## 4. 일별 Granger — ΔVIX ↔ BTC (양방향)")
w("### 4.1 All 표본")
R["d_v2b"]=block("VIX→BTC","dvix_pct→",None,"ΔVIX%","BTC수익률",MAXLAG_D) if False else block("VIX→BTC",dd["dvix_pct"],dd["btc_ret"],"ΔVIX%","BTC수익률",MAXLAG_D)
R["d_b2v"]=block("BTC→VIX",dd["btc_ret"],dd["dvix_pct"],"BTC수익률","ΔVIX%",MAXLAG_D)
pre=dd[dd.year<2024]; post=dd[dd.year>=2024]
w(f"### 4.2 pre-ETF (2020~2023, n={len(pre)})")
block("VIX→BTC(pre)",pre["dvix_pct"],pre["btc_ret"],"ΔVIX%","BTC수익률",MAXLAG_D)
block("BTC→VIX(pre)",pre["btc_ret"],pre["dvix_pct"],"BTC수익률","ΔVIX%",MAXLAG_D)
w(f"### 4.3 post-ETF (2024~, n={len(post)})")
R["dp_v2b"]=block("VIX→BTC(post)",post["dvix_pct"],post["btc_ret"],"ΔVIX%","BTC수익률",MAXLAG_D)
block("BTC→VIX(post)",post["btc_ret"],post["dvix_pct"],"BTC수익률","ΔVIX%",MAXLAG_D)

w("## 5. 일별 Granger — VIX slope(기간구조) ↔ BTC (양방향)")
ds=dd.dropna(subset=["dslope"])
w(f"> slope 데이터(`vix_slope_daily.parquet`)는 **2024-01 이후 {len(ds)}일만 존재**(post-ETF 구간). "
  f"따라서 slope 분석은 post-ETF 레짐 검정에 해당한다. slope = VIX(3m) − VIX(30d).")
w("")
...
for ax,keys,title,xl in [
    (axes[0],[("d_v2b","VIX->BTC full","o-"),("d_b2v","BTC->VIX full","s-"),("dp_v2b","VIX->BTC post","o--")],"Daily dVIX Granger p","lag(days)"),
...
row("ΔVIX→BTC (일별 All)","d_v2b"); row("BTC→ΔVIX (일별 All)","d_b2v")
row("ΔVIX→BTC (일별 post)","dp_v2b")
row("slope→BTC (post-ETF, 2024+)","s_v2b"); row("BTC→slope (post-ETF, 2024+)","s_b2v")
w(f"| ΔVIX→BTC (시간별 명목) | {min(R['h_v2b'].values()):.4f} | {R['h_v2b'][1]:.4f} | ⚠️ 동시성 누수(6.1) |")
w(f"| **ΔVIX→BTC (시간별 진짜예측)** | {r2[1]:.4f} | — | **예측력 없음** |")
w("")
w("### 최종 해석")
w("- **일별 ΔVIX·slope 모두**: 양방향·전 레짐에서 시차 예측력 **없음**(Bonferroni 임계 통과 0건).")
