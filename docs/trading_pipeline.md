...
| BTC 1분봉 (체결 기반) | ✅ | ✅ | ✅ |
| BTC LOB (호가창) | ❌ | ❌ | ✅ |
| VIXY 1분봉 | ✅ (수신 완료) | ✅ (수신 완료) | 확인 필요 |
| VIX 일봉 | ✅ | ✅ | ✅ |
| HMM 방향 signal | 타 팀원 제공 | 타 팀원 제공 | 타 팀원 제공 |
| LSTM-AE anomaly signal | ✅ | ✅ | 재학습 후 생성 |
| BTC 펀딩비 | 미수집 | 미수집 | 미수집 |
...
| 항목 | 현재 | VIXY 합류 후 |
|---|---|---|
| anomaly 시 행동 | 미확정 | 방향 예측 가능 시 → 전환, 불가 시 → 청산 |
| 최소 보유 시간 | 5분 (잠정) | 데이터 기반 재검토 |
| 임계값 방식 | 단일 (92.5%ile) | 히스테리시스(이중 임계값) 검토 |

### 수집 필요 데이터

| 데이터 | 소스 | 비고 |
|---|---|---|
| BTC 펀딩비 | ccxt (Binance) | 8시간 단위, 전 구간 필요 |
